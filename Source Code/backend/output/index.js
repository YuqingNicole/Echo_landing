...
var src_default = {
  async fetch(request, env2, ctx) {
    ...
    try {
      ...
      console.log("\u{1F4CB} Request Info:", {
        path,
        method,
      ...
      if (path === "/api/save-profile" && method === "POST") {
        return await handleSaveProfile(request, env2, userId);
      }
      if (path === "/api/get-profile" && method === "GET") {
        return await handleGetProfile(request, env2, userId);
      }
      if (path === "/api/save-test-results" && method === "POST") {
        return await handleSaveTestResults(request, env2, userId);
      }
      if (path === "/api/get-matches" && method === "GET") {
        return await handleGetMatches(request, env2, userId);
      }
      if (path === "/api/save-matches" && method === "POST") {
        return await handleSaveMatches(request, env2, userId);
      }
...
async function handleSaveProfile(request, env2, userId) {
  const corsHeaders = {
    "Access-Control-Allow-Origin": "*",
    "Access-Control-Allow-Methods": "GET, POST, PUT, DELETE, OPTIONS",
    "Access-Control-Allow-Headers": "Content-Type, Authorization, X-Requested-With"
  ...
  try {
    ...
    console.log("\u{1F4BE} Saving profile for user:", userId);
    ...
    console.error("\u274C Failed to save profile:", error3);
    return new Response(JSON.stringify({
      success: false,
...
__name(handleSaveProfile, "handleSaveProfile");
async function handleGetProfile(request, env2, userId) {
  const corsHeaders = {
    "Access-Control-Allow-Origin": "*",
    "Access-Control-Allow-Methods": "GET, POST, PUT, DELETE, OPTIONS",
    "Access-Control-Allow-Headers": "Content-Type, Authorization, X-Requested-With"
  ...
  try {
    console.log("\u{1F4D6} Getting profile for user:", userId);
    ...
    console.error("\u274C Failed to get profile:", error3);
    return new Response(JSON.stringify({
      success: false,
...
async function handleSaveTestResults(request, env2, userId) {
  const corsHeaders = {
    "Access-Control-Allow-Origin": "*",
    "Access-Control-Allow-Methods": "GET, POST, PUT, DELETE, OPTIONS",
    "Access-Control-Allow-Headers": "Content-Type, Authorization, X-Requested-With"
  ...
  try {
    ...
    console.log("\u{1F3AF} Saving test results for user:", userId);
    ...
    console.error("\u274C Failed to save test results:", error3);
    return new Response(JSON.stringify({
      success: false,
...
__name(handleSaveTestResults, "handleSaveTestResults");
async function handleGetMatches(request, env2, userId) {
  if (!userId) {
    return new Response(JSON.stringify({
      success: false,
      error: "User not authenticated"
  ...
  try {
    console.log("\u{1F50D} Getting matches for user:", userId);
    const user = await env2.DB.prepare(
      "SELECT id FROM users WHERE encrypted_yw_id = ?"
    ).bind(userId).first();
    if (!user) {
    ...
    console.error("\u274C Failed to get matches:", error3);
    return new Response(JSON.stringify({
      success: false,
...
async function handleSaveMatches(request, env2, userId) {
  if (!userId) {
    return new Response(JSON.stringify({
      success: false,
      error: "User not authenticated"
  ...
  try {
    ...
    console.log("\u{1F49D} Saving matches for user:", userId);
    const user = await env2.DB.prepare(
      "SELECT id FROM users WHERE encrypted_yw_id = ?"
    ).bind(userId).first();
    if (!user) {
    ...
    console.error("\u274C Failed to save matches:", error3);
    return new Response(JSON.stringify({
      success: false,
...
__name(handleSaveMatches, "handleSaveMatches");
export {
